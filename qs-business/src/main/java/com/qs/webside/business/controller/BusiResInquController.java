...
/**
 * 商务业绩查询
 * Created by zun.wei on 2017/4/27 10:12.
 */
@Controller
@RequestMapping(value = "/business/")
public class BusiResInquController extends BaseController {

    // /business/businessResultsInquiriesUi.html
    @Resource
     ...
     * @description:商务业绩查询入口
     * @param model
     * @param request
     * @return
     */
    @RequestMapping(value = "businessResultsInquiriesUi.html",method = RequestMethod.GET)
    public String businessResultsInquiriesUi(Model model, HttpServletRequest request){
        String gameName = appGameService.getGameName(businessDataSourceUtil.getGameType());
        PageUtil page = super.getPage(request);
        model.addAttribute("page", page);
        model.addAttribute("gameName", gameName);
        return "WEB-INF/view/web/business/business_ResInquSearch_list";
    }
     ...
     */
    @ResponseBody
    @RequestMapping(value = "businessResultsInquiries.html", method = RequestMethod.POST)
    public Object businessResultsInquiries(String gridPager, HttpServletResponse response) throws Exception {
        ...
        if(pager.getIsExport()){//判断是否是导出操作
            if(pager.getExportAllData()){
                //3.1、导出全部数据
                List<Map<String,Object>> list = playerPayDayService.queryBusiPayByCompany(parameters);
