...
public class ActiAwardRecordController extends BaseController {
    ...
    public Object addExchangePrizes(HttpServletRequest request, BaseRequest baseRequest) {
        ...
        if (actiAward.getStock().intValue() < number) return this.getReturnData(null, AppConstants.Result.FAILURE_1012);//库存不足
        if (!"1".equals(actiAward.getRemark())) {//如果不是兑换房卡的商品
            ActiAwardAddress actiAwardAddress = actiAwardAddressService.selectObjectByMidKey(mid);//收获地址
        ...
        return verifyPrizeQuantity(number, mid, actiAward, hashExAwards, awards,baseRequest.getSesskey());
    }
     ...
     * @Description:验证奖品数量
     */
    private Object verifyPrizeQuantity(int number, int mid, ActiAward actiAward, long hashExAwards, long awards, String sesskey) {
        ...
        if ((awards - hashExAwards) >= number) {//判断奖品数量是否充足
            ...
            return verifyIntegralObject(number, mid, actiAward, actiIntegral,sesskey);
        } else {//奖品不足
     ...
     */
    private Object verifyIntegralObject(int number, int mid, ActiAward actiAward, ActiIntegral actiIntegral, String sesskey) {
        if (actiIntegral != null) {
            long nowIntegral = actiIntegral.getNowIntegral();
            long needIntegral = actiAward.getIntegral();
            return verifyIntegral(number, mid, actiAward, actiIntegral, nowIntegral, needIntegral,sesskey);
        } else {//当前人没有积分对象
     ...
     */
    private Object verifyIntegral(int number, int mid, ActiAward actiAward, ActiIntegral actiIntegral, long nowIntegral, long needIntegral, String sesskey) {
        if (nowIntegral >= needIntegral * number) {//比较现存积分与需要的积分值
            Map<String, Object> parameters = new HashMap<>();
            parameters.put("id", actiAward.getId());
            parameters.put("reduce", number);
            ...
            if (updateStockResult > 0) {//更新库存成功
                ...
                return updateInegral(number, mid, actiAward, resultAddIntegral,sesskey);
            } else {//更新库存失败，说明没有库存了。
                actiAwardService.deleteAwardStockCache(actiAward.getId());//删除对应商品的库存的缓存
                return this.getReturnData(null, AppConstants.Result.FAILURE_1011);
            }
        } else {//积分不足
     ...
     * @Description:更新积分
     */
    private Object updateInegral(int number, int mid, ActiAward actiAward, int resultAddIntegral, String sesskey) {
        if (resultAddIntegral > 0) {//更新积分成功
            int result = actiAwardRecordService.insertSelective(preparationRecord(mid, actiAward, number));
            return addActiAwardRecord(mid, actiAward, result,sesskey);
        } else {//更新积分失败
     ...
     */
    private Object addActiAwardRecord(int mid, ActiAward actiAward, int result, String sesskey) {
        if (result > 0) {//插入奖品兑换记录表成功
            return exchangeRoomCard(mid, actiAward, result,sesskey);
        } else {//插入奖品兑换记录表失败
