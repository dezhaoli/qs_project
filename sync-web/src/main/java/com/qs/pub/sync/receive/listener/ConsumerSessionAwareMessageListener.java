...
import com.qs.pub.sync.receive.mq.IReceiveDataCenter;
...
public class ConsumerSessionAwareMessageListener implements SessionAwareMessageListener<ActiveMQObjectMessage> {
	...
	private IReceiveDataCenter receiveDataCenter;
	public void setReceiveDataCenter(IReceiveDataCenter receiveDataCenter){
		this.receiveDataCenter = receiveDataCenter;
	}

	public synchronized void onMessage(ActiveMQObjectMessage message,Session session) {
		try {
			receiveDataCenter.handleMessage(message,session);
		} catch (Exception e) {
			e.printStackTrace();
