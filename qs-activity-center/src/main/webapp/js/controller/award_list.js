define(['app',"layer"],function(app){
    app.controller('ctrl.index',function($scope,$location,$http,$myService,$state,$timeout,$window,$state ){
        	...
        	$.post(url+"/api/actiIntegral/getRankingByUserMid.do", parma,
                	...
                	if (result.svflag==1){
                		$timeout(function(){  
                            ...
                            sessionStorage.nowIntegral=JSON.parse(data).data.nowIntegral;
                		},100);
                	}else if (result.svflag==1001){
                		$scope.nowIntegral=0;	
                	}else {
        ...
        if (sessionStorage.nowIntegral){
            $scope.nowIntegral= sessionStorage.nowIntegral;
        }else  {
            $scope.getuserinfo();
        }
            	...
            	layer.close(index);
            	 var type=0;
              	...
              	$.post(url+"/api/actiAwardRecord/exchangePrizes.do", par,
                  	...
                  	}else if(result.svflag==1012 || result.svflag==1011){
                        layer.msg("抱歉，暂无库存！", { icon : 5, time : 1000 });
