 ...
 */
@Controller
@RequestMapping(value = "/majiangjx/")
public class MajiangjxController extends PayBaseController {

	Logger log = Logger.getLogger(MajiangjxController.class);
    ...
    @Value("${majiangjx.appid}")
    private  String appid;

    @Value("${majiangjx.mchid}")
    private  String mchid;

    @Value("${majiangjx.key}")
    private  String apikey;

    @Value("${majiangjx.certfile}")
    ...
    public String agentWeekInfoStaUi(Model model, HttpServletRequest request) {
        try {
            ...
            model.addAttribute("gameType", ConstantUtil.GameTypeConstant.Ma_Jiangjx);
    ...
    public Object agentWeekInfoSta(String gridPager, HttpServletResponse response) throws Exception {
        DataSourceSwitch.setLogDataSourceType(ConstantUtil.LogDataSourceConstantKey.Ma_Jiangjx_Log);
        DataSourceSwitch.setMainDataSourceType(ConstantUtil.MainDataSourceConstantKey.Ma_Jiangjx_Main);
        Map<String,Object> map = super.getAgentWeekInfoSta(gridPager, response,
                ConstantUtil.GameTypeConstant.Ma_Jiangjx, taxesInviteWeekService);
        return map;
    }
    ...
    public Object confirmPay(String openid, Date date, Integer mid,HttpServletRequest request){
    	...
    	if(ConstantUtil.NoPay.MAJIANXJX_MID.contains(mid+"")){
			map.put(CommonContants.SUCCESS, Boolean.FALSE);
			map.put(CommonContants.MESSAGE, "不需要返利的代理商");
			  return map;
		}
		try {
			map = super.weixinPay(request,mid, date,PAY, taxesInviteWeekService, memberAgentService, payLog,ConstantUtil.GameType.Ma_Jiangjx);
		} catch (WeixinException e) {
			map.put(CommonContants.SUCCESS, Boolean.FALSE);
			map.put(CommonContants.MESSAGE, e.getMessage());
			this.saveWeinMsg(mid,weinxinMsgService,ConstantUtil.GameType.Ma_Jiangjx, e);
			e.printStackTrace();
		}catch (Exception e) {   
			map.put(CommonContants.SUCCESS, Boolean.FALSE);
    ...
    public Object saveSimplePay(int mid,BigDecimal money,String msg, HttpServletRequest request){
          ...
          Map<String, Object> map = new HashMap<String, Object>();
		try {
			map = super.saveSimplePay(request,mid, money, msg,PAY, taxesInviteWeekService, memberAgentService, payLog,ConstantUtil.GameType.Ma_Jiangjx);
		} catch (WeixinException e) {
			map.put(CommonContants.SUCCESS, Boolean.FALSE);
			map.put(CommonContants.MESSAGE, e.getMessage());
			this.saveWeinMsg(mid,weinxinMsgService,ConstantUtil.GameType.Ma_Jiangjx, e);
    ...
    public void exportDetailExcel(String gridPager,HttpServletResponse response,String date,String stadate) throws Exception {
        DataSourceSwitch.setLogDataSourceType(ConstantUtil.LogDataSourceConstantKey.Ma_Jiangjx_Log);
        DataSourceSwitch.setMainDataSourceType(ConstantUtil.MainDataSourceConstantKey.Ma_Jiangjx_Main);
        super.DetailExcelExport(stadate, ConstantUtil.GameTypeConstant.Ma_Jiangjx,date, response, taxesInviteWeekService);
    }
    ...
    public Object batchPay(HttpServletRequest request,String searchDate){
        ...
        parameters.put("dbTable", ConstantUtil.GameTypeConstant.Ma_Jiangjx + ".memberagents");
            ...
            for(TaxesInviteWeek record:listInvite){
                ...
                try {
					Map<String,Object> paymap = super.weixinPay(request,mid, date, PAY, taxesInviteWeekService,
					        memberAgentService, payLog,ConstantUtil.GameType.Ma_Jiangjx);
				} catch (WeixinException e) {
					map.put(CommonContants.SUCCESS, Boolean.FALSE);
					map.put(CommonContants.MESSAGE, e.getMessage());
					this.saveWeinMsg(mid,weinxinMsgService,ConstantUtil.GameType.Ma_Jiangjx, e);
