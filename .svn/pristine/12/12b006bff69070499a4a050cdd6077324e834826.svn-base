...
function confrimPay(openid, date, mid,rebatetotal) {
    ...
    if (rebatetotal > 200) {
        ...
        layer.confirm('确认要支付吗？', {
            ...
            $.ajax({
                type : "POST",
                url : sys.rootPath +"/corpPay/confirmPay.html",
                data : {
                    openid:openid,
                    date:date,
                    gameType:gameType,
...
$('#oneKeyPay').on('click',function () {
    ...
    layer.open({
        content: '确认要一键支付'+first + '--' + last+'吗？,这个操作大概需要几十秒钟，系统提示支付成功后，再进行操作，切勿多次点击确定、防止重复提交(提示：多个账号不要同时使用一键支付) !'
        ,btn: ['确定', '取消']
        ,yes: function(index, layero){
            ...
            $.ajax({
                type : "POST",
                url : sys.rootPath  +"/corpPay/batchPay.html",
                data : {
...
var dtGridOption = {
    ...
    checkWidth :'37px',
    extraWidth : '37px',
    loadURL : sys.rootPath + '/corpPay/agentCorpPayList.html?gameType='+gameType,
...
$(function() {
    ...
    grid.parameters['gameType'] = $('#gameType').val();
