...
public class BullController extends PayBaseController {
	...
	Logger log = Logger.getLogger(BullController.class);
    @Resource
    ...
    public Object agentWeekInfoSta(String gridPager, HttpServletResponse response) throws Exception {
        ...
        return super.getAgentWeekInfoSta(gridPager, response,
                ConstantUtil.GameTypeConstant.Bull, taxesInviteWeekService);
    }
    ...
    public Object confirmPay(String openid, Date date, Integer mid,HttpServletRequest request){
    	...
    	if(ConstantUtil.NoPay.BULL_MID.contains(mid+"")){
			map.put(CommonContants.SUCCESS, Boolean.FALSE);
			map.put(CommonContants.MESSAGE, "不需要返利的代理商");
			  return map;
		}
        try {
        	map=super.weixinPay(request,mid, date,PAY,taxesInviteWeekService, memberAgentService, payLog,ConstantUtil.GameType.Bull);
		} catch (WeixinException e) {
			map.put(CommonContants.SUCCESS, Boolean.FALSE);
			map.put(CommonContants.MESSAGE, e.getMessage());
			this.saveWeinMsg(mid,weinxinMsgService,ConstantUtil.GameType.Bull, e);
			e.printStackTrace();
		}catch (Exception e) {   
			map.put(CommonContants.SUCCESS, Boolean.FALSE);
    ...
    public Object saveSimplePay(int mid,BigDecimal money,String msg, HttpServletRequest request) {
        ...
        Map<String, Object> map = new HashMap<String, Object>();
		try {
			map = super.saveSimplePay(request,mid, money, msg, PAY, taxesInviteWeekService, memberAgentService, payLog,ConstantUtil.GameType.Bull);
		} catch (WeixinException e) {
			map.put(CommonContants.SUCCESS, Boolean.FALSE);
			map.put(CommonContants.MESSAGE, e.getMessage());
			this.saveWeinMsg(mid,weinxinMsgService,ConstantUtil.GameType.Bull, e);
    ...
    public void exportDetailExcel(String gridPager,HttpServletResponse response,String date,String stadate) throws Exception {
        DataSourceSwitch.setLogDataSourceType(ConstantUtil.LogDataSourceConstantKey.Bull_Log);
        DataSourceSwitch.setMainDataSourceType(ConstantUtil.MainDataSourceConstantKey.Bull_Main);
        super.DetailExcelExport(stadate, ConstantUtil.GameTypeConstant.Bull,date, response, taxesInviteWeekService);
    }
			...
			for(TaxesInviteWeek record:listInvite){
			    ...
			    try {
					Map<String,Object> paymap = super.weixinPay(request,mid, date, PAY, taxesInviteWeekService,
					        memberAgentService, payLog,ConstantUtil.GameType.Bull);
				} catch (WeixinException e) {
					map.put(CommonContants.SUCCESS, Boolean.FALSE);
					map.put(CommonContants.MESSAGE, e.getMessage());
					this.saveWeinMsg(mid,weinxinMsgService,ConstantUtil.GameType.Bull, e);
