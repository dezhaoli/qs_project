drop procedure IF EXISTS updateTaxesInviteWeekTime;
delimiter //
CREATE DEFINER=`jxadmin`@`%` PROCEDURE `updateTaxesInviteWeekTime`()
BEGIN
...
SELECT mid,parent_id,scale,mktime,belongid FROM jx_majiang.memberagents WHERE status=0;
...
(case  when b.paytotal2>0 then b.paytotal2*0.15 else 0 end) proceeds2 ,
(case  when c.paytotal3>0 then c.paytotal3*0 else 0 end) proceeds3
from (
SELECT SUM(paytotal-selftotal) AS paytotal1,SUM(invitetotal) AS invitetotal1 FROM taxes_invite WHERE date>="2017-07-17" AND date<="2017-07-23" AND mid=@mid ) a,
(SELECT SUM(paytotal) AS paytotal2,SUM(invitetotal) AS invitetotal2 FROM taxes_invite WHERE date>="2017-07-17" AND date<="2017-07-23" AND parentid=@mid) b,
...
i.date>="2017-07-17" AND i.date<="2017-07-23" ) c ) p;

DECLARE directlyDay CURSOR FOR
SELECT SUM(paycount) AS paycount,SUM(playtimes) AS playtimes,SUM(bindpeople) AS bindpeople FROM taxes_directly_day WHERE mid=@mid AND date>="2017-07-17" AND date<="2017-07-23";
...
bindpeople=countinvitotal,date="2017-07-23",info=allInfo;

INSERT IGNORE INTO jx_majiang_pub.biz_invite_week SET bizid=tbelongid,date="2017-07-23",paytotal=counttotal,gametype=6,rebatetotal=countceeds,bindpeople=countinvitotal
...
INSERT IGNORE INTO taxes_directly_week SET mid=@mid,bindpeople=fistInviteCount,paycount=fistPayCount,playtimes=fistPayTimes,rebatetotal=fistInviteCount+fistPayCount,date="2017-07-23";
end if;
