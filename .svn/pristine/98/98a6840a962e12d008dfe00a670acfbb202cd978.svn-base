...
$(function() {
        $('#companyTree').jstree({
            "core" : {
                'data' : {
                    "url" : sys.rootPath + "/sysCompany/resourceTree.html?companyId=" + $("#companyId").val(),
                    "dataType" : "json"
                },
                "themes" : {
                    "responsive" : true
...
<div class="page-header">
	<h1>
		数据权限设置-<small>[${companyEntity.companyName }]</small>
	</h1>
</div>
<div class="row" style="margin-top:5px;">
	<div class="col-xs-12">
		...
		<input type="hidden" id="companyId" name="companyId" value="${companyEntity.id }">
		   <!-- 资源树 -->
		   <div class="form-group">
		      <label class="control-label col-sm-1 no-padding-right" for="description">用户组数据列表</label>
		      <div class="col-sm-10">
...
<div class="center">
	<button id="btnAdd" type="button" onclick="commitCompany('/sysCompany/permission.html' , '/sysCompany/toCompanyListUi.html')" class="btn btn-success btn-sm">
	  	<i class="fa fa-user-plus"></i>&nbsp;
		保存
	</button>
		<button id="btn" type="button" onclick="webside.common.loadPage('/sysCompany/toCompanyListUi.html?page=${page.pageNum }&rows=${page.pageSize }&sidx=${page.orderByColumn }&sord=${page.orderByType }')" class="btn btn-info btn-sm">
...
<script>
var ids = [];
commitCompany = function(commitUrl, listUrl) {
	debugger;
    var tree = $('#companyTree').jstree();
    //获取所有选中节点id
    ...
    for (var i = 0; i < selected.length; i++) {
    	getCompanyParents(tree, selected[i]);
    }
    var index;
    $.ajax({
        type : "POST",
...
getCompanyParents = function(treeObj, nodeObj) {
