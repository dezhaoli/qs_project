...
function showDetail(date,stadate) {
    layer.open({
        ...
        fixed: false, //不固定
        maxmin: true,
        content: sys.rootPath + '/settle/agentWeekInfoStaHistoryDetailUi.html?date='+
        date+'&stadate='+stadate
    });
}
...
function exportExcel(date,stadate) {
    layer.confirm('确认要导出Excel吗？', {
        ...
        $('#iframeOut').attr("src",sys.rootPath + "/settle/exportDetailExcel.html?date="
            + date + "&stadate="+stadate+ "_结算审核详情");
...
var dtGridOption = {
    ...
    checkWidth :'37px',
    extraWidth : '37px',
    loadURL : sys.rootPath + '/settle/agentWeekInfoStaHistory.html',
    columns : dtGridColumns,
    gridContainer : 'dtGridContainer',
    toolbarContainer : 'dtGridToolBarContainer',
    tools : 'refresh',//'refresh|print|export[excel,csv,pdf,txt]'
    exportFileName : '代理商历史结算审核列表',
    pageSize : pageSize,
