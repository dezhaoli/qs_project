drop procedure IF EXISTS updateTaxesInviteWeek001;
delimiter //
CREATE  PROCEDURE `updateTaxesInviteWeek001`()
BEGIN
...
from (
SELECT SUM(paytotal-selftotal) AS paytotal1,SUM(invitetotal) AS invitetotal1 FROM taxes_invite WHERE date>="2017-07-10" AND date<="2017-07-16" AND mid=@mid ) a,
(SELECT SUM(paytotal) AS paytotal2,SUM(invitetotal) AS invitetotal2 FROM taxes_invite WHERE date>="2017-07-10" AND date<="2017-07-16" AND parentid=@mid) b,
...
i.date>="2017-07-10" AND i.date<="2017-07-16" ) c ) p;

DECLARE directlyDay CURSOR FOR
SELECT SUM(paycount) AS paycount,SUM(playtimes) AS playtimes,SUM(bindpeople) AS bindpeople FROM taxes_directly_day WHERE mid=@mid AND date>="2017-07-10" AND date<="2017-07-16";
...
bindpeople=countinvitotal,date="2017-07-16",info=allInfo;

INSERT IGNORE INTO jx_majiang_pub.biz_invite_week SET bizid=tbelongid,date="2017-07-16",paytotal=counttotal,gametype=6,rebatetotal=countceeds,bindpeople=countinvitotal
...
INSERT IGNORE INTO taxes_directly_week SET mid=@mid,bindpeople=fistInviteCount,paycount=fistPayCount,playtimes=fistPayTimes,rebatetotal=fistInviteCount+fistPayCount,date="2017-07-16";
end if;
