...
public class AgentStoreController extends BaseController {
    ...
    public String listUI(Model model, HttpServletRequest request) {
        try {
            ...
            return "/WEB-INF/view/cfg/acti/agent_store_list";
        } catch (Exception e) {
    ...
    public Object list(String gridPager) throws Exception {
        ...
        List<AgentStore> list = agentStoreSerivce.queryListByPage(parameters);
    ...
    public String addUI(Model model, HttpServletRequest request, String id) {
        try {
            ...
            model.addAttribute("page", page);
            return "/WEB-INF/view/cfg/acti/agent_store_form";
        } catch (Exception e) {
            throw new SystemException(e);
        }
    }
    ...
    public String editUI(Model model, HttpServletRequest request, Integer id) {
        try {
            AgentStore record = agentStoreSerivce.findById(id);
            PageUtil page = new PageUtil(request);
            model.addAttribute("imgUploadUrl", imgUploadUrl);
            model.addAttribute("page", page);
    ...
    @ResponseBody
    public Object add(AgentStore record) {
        ...
        try {
            ...
            int result = agentStoreSerivce.insert(record);
            if (result > 0) {
    ...
    @ResponseBody
    public Object update(AgentStore record) {
        record.setStarttime(DateUtil.convertToInt(record.getStarttimeStr()));
        record.setEndtime(DateUtil.convertToInt(record.getEndtimeStr()));
        Map<String, Object> map = new HashMap<String, Object>();
        try {
            //stockistLove.setUpdateTime(new Date(System.currentTimeMillis()));
            int result = agentStoreSerivce.update(record);
            if (result > 0) {
                map.put(CommonContants.SUCCESS, Boolean.TRUE);
                map.put(CommonContants.DATA, null);
    ...
    public Object deleteById(Integer id) {
        Map<String, Object> map = new HashMap<String, Object>();
        try {
            int result = agentStoreSerivce.deleteById(id);
            if (result > 0) {
