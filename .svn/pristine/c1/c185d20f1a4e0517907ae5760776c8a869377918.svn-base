...
//				String accountName="ogl_xwv2CcSdAFbgMptWiuZ3X9yg";//"ogl_xwv4KKiJuyb0mAdK0OFzlBBQ";
		...
		public String refreshCache(int gameType,String sitemid){
			...
			if (gameType ==0 ||StringUtils.isBlank(sitemid)){
				return "game_list";
			}
	...
	public String home(Model model) {
		try
		{
			MemberAgents memberAgents = (MemberAgents)SecurityUtils.getSubject().getPrincipal();
			agentDataSourceUtil.setReadDataSourceType();			
			MemberInvite memberInvite = memberInviteMapper.selectBySiteId(memberAgents.getSitemid());
	...
	public Map<String,Object> bindPhoneUi(String phone){
		...
		MemberAgents memberAgents = (MemberAgents)SecurityUtils.getSubject().getPrincipal();
		String sitemid=CommonUtils.checkNull(memberAgents.getSitemid());
	    CommonAgents	commonAgent=commonAgentService.selectByPhoneInfo(phone.trim());
	   if (commonAgent ==null||sitemid.equals(commonAgent.getSitemid())){
