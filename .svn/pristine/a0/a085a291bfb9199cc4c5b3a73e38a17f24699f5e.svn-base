/*
 * 文件名：ICreateRoomService.java	 
 * 时     间：下午5:21:58
 * 作     者：wangzhen      
 * 版     权：2014-2022  牵手互动, 公司保留所有权利.
 * 
 */
package com.qs.pub.datacenter.service;

import java.util.List;
import java.util.Map;

import com.qs.pub.datacenter.model.CreateRoom;

/** 
 * @ClassName: ICreateRoomService 
 * @描述: (这里用一句话描述这个类的作用) 
 * @author qs
 * @date 2017年5月23日 下午5:21:58 
 */
public interface ICreateRoomService
{
	int insert(CreateRoom createRoom);

	List<CreateRoom> queryPageList(Map<String, Object> parameters);

	List<CreateRoom> queryCount(Map<String, Object> parameters);

	Long queryCreateRoomCountTotal(Map<String, Object> parameters);

	List<CreateRoom> queryListSecondDetails(Map<String, Object> parameters);

	List<CreateRoom> queryListOfExport(Map<String, Object> parameters);

    List<CreateRoom> queryListThreeDetails(Map<String, Object> parameters);

    List<CreateRoom> queryListFourDetails(Map<String, Object> parameters);
}
