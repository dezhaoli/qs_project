		    ...
		    <div class="form-group">
		      ...
		      <div class="clearfix">
		           	<select class="form-control" id="type" name="type" style="width: 100%">
						<option value="1" <c:if test="${record.type eq '1' }">selected="selected"</c:if>>血流成河</option>
						<option value="2" <c:if test="${record.type eq '2' }">selected="selected"</c:if>>血战到底</option>
					</select>
		      </div>
		      </div>
