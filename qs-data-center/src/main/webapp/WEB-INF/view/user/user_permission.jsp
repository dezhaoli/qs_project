...
$(function() {
        $('#userTree').jstree({
            "core" : {
                'data' : {
                    "url" : sys.rootPath + "/user/resourceTree.html?userId=" + $("#userId").val(),
                    "dataType" : "json"
                },
                "themes" : {
                    "responsive" : true
...
<div class="page-header">
	<h1>
		数据权限设置-<small>[${userEntity.userName }]</small>
	</h1>
</div>
...
<div class="center">
	<button id="btnAdd" type="button" onclick="commitUser('/user/permission.html' , '/user/listUI.html')" class="btn btn-success btn-sm">
	  	<i class="fa fa-user-plus"></i>&nbsp;
		保存
	</button>
		<button id="btn" type="button" onclick="webside.common.loadPage('/user/listUI.html?page=${page.pageNum }&rows=${page.pageSize }&sidx=${page.orderByColumn }&sord=${page.orderByType }')" class="btn btn-info btn-sm">
...
<script>
var ids = [];
commitUser = function(commitUrl, listUrl) {
	debugger;
    var tree = $('#userTree').jstree();
    //获取所有选中节点id
