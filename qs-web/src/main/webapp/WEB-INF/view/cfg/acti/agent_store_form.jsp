...
<div class="center">
	<button id="btnAdd" type="button" onclick="javascript:$('#storeForm').submit();" class="btn btn-success btn-sm">
		...
		<button id="btn" type="button" onclick="webside.common.loadPage('/cfg/storeAgent/listUI.html<c:if test="${!empty record}">?page=${page.pageNum }&rows=${page.pageSize }&sidx=${page.orderByColumn }&sord=${page.orderByType }</c:if>')" class="btn btn-info btn-sm">
...
$(function() {
    ...
    $('#storeForm').validate({
        ...
        submitHandler : function(form) {
            ...
            var url = "";
            if (id != undefined) {
                url = '/cfg/storeAgent/edit.html';
            } else {
                url = '/cfg/storeAgent/add.html';
            }
            webside.common.commit('storeForm', url, '/cfg/storeAgent/listUI.html');
        }
    });
});
