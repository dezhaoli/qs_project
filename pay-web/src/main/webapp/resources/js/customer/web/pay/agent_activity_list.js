...
function confrimPay(openid, date, mid,rebatetotal) {
    ...
    if (rebatetotal > 200) {
        ...
        layer.confirm('确认要支付吗？', {
            ...
            $.ajax({
                type : "POST",
                url : sys.rootPath + "/corpPay/activityPay.html",
                data : {
                    openid:openid,
                    date:date,
                    gameType:gameType,
...
$('#oneKeyPay').on('click',function () {
    ...
    layer.open({
        content: '确认要一键支付吗？,这个操作大概需要几十秒钟，系统提示支付成功后，再进行操作，切勿多次点击确定、防止重复提交!'
        ,btn: ['确定', '取消']
        ,yes: function(index, layero){
            ...
            $.ajax({
                type : "POST",
                url : sys.rootPath + "/corpPay/activityBatchPay.html",
                data : {
...
var dtGridOption = {
    ...
    exportFileName : '活动支付' + lastMonday + "--" + lastSunday,
    pageSize : pageSize,
